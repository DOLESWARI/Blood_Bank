...
class d_report:
    ...
    def pdf_state(self):
        ...
        count=1
        p = canvas.Canvas("Full_Donor_Report.pdf", pagesize=portrait(A4))
        #self.pdf_layout(p)
        ...
        for i in range(len(lst)):
            ...
            name_lst = self.c.execute("select donor.did,dname,state,blood_grp,quantity,date from Donor,donate where donate.did=donor.did and donor.state=? order by donor.did", (lst[i][0],))
